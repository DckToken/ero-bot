...
async def on_message(message):
    ...
    if message.content.startswith('!search '):
        ...
        if error is True:
            await bot.send_message(message.channel, 'Not found! Try an exact show name, please.')
        elif error is False:
            print(num) #Debug
            print(line) #Debug
            show = arg
            ...
            choice = 1
            p = 1
            await bot.send_message(message.channel, 'Show found, check your DMs!')
            await bot.send_message(message.author, 'Found ``' + str(files) + '`` doujinshi')
            await bot.send_typing(message.author)
            pages = (int(files / 11)) + 1
            await bot.send_message(message.author, '[DEBUG] Pages: ' + str(pages))
            if pages == 1:
                ...
                while i < num + files:
                    ...
                    await bot.send_message(message.author, '``' +str(choice) + '``) ' + name + ' (``' + doujinshifiles + '`` pages)')
                    await bot.send_typing(message.author)
                    ...
                    if choice == files:
                        break
                        await bot.send_message(message.author, 'Type a selection to continue...')
                        pick = await client.wait_for_message(timeout=20.0, author=message.author)
                        if pick is None:
                            await bot.send_message(message.author, 'Timed out, try again!')
                            return
                ...
                while currentpage <= pages:
                    ...
                    while choice < currentpage * 10 + 1:
                        await bot.send_message(message.author, 'Page number ``' + str(currentpage) + '``/``' + str(pages) + '``:')
                        while i < num + files: #for kill la kill is "i < 68 + 13" (which is 81, last line) so this is :ok_hand:
                            ...
                            if choice > currentpage * 10 or choice == files + 1 and pagesleft == 0:
                                await bot.send_message(message.author, '**Last page reached.**\nType a selection to continue or type ``exit`` to cancel')
                                pick = await client.wait_for_message(timeout=20.0, author=message.author)
                                if pick is None:
                                    await bot.send_message(message.author, 'Timed out, try again!')
                                    return
    ...
    if message.content.startswith('!listshows'):
        await bot.send_message(message.channel, 'Shows in my database: (this might take a while)')
        i = 0
        with open('list.txt') as f:
            lines=f.readlines()
            ...
            while l < num_lines + 1:
                ...
                await bot.send_message(message.channel, "**·** ``" + name + '`` (``' + files + '`` doujinshis!)')
                l = l + int(files) + 2
                i = i + 1
                try:
                ...
                if howdoicallthis > pages:
                    await bot.send_message(message.channel, '**Page end reached**.\nIf you want to countinue type ``next``. If you want to stop, type ``exit`` instead!')
                    pick = await client.wait_for_message(timeout=20.0, author=message.author)
                    if pick.content == "exit":
                        await bot.send_message(message.channel, 'If you want a show added, use ``!suggest <show name>``')
                        return
                    if pick.content == None:
                        await bot.send_message(message.author, 'Timed out, automatically exited!')
                        return
                    if pick.content.lower() == "next":
                        continue
                else:
    ...
    if message.content.startswith('!suggest '):
        ...
        await bot.send_message(message.channel, 'Running script, searching for ``' + arg + '``...')
        try:
            subprocess.run(["bash", "/media/ero-bot/unpack.bash", arg], cwd="/media/ero-bot", check=True)
        except subprocess.CalledProcessError:
            print('error on script')
            await bot.send_message(message.channel, "Bash script error: Couldn't find ``" + arg + '`` Or there was a problem extracting. Try another show name.')
        else:
            await bot.send_message(message.channel, 'Doujinshi ``' + arg + '`` successfully added!')
    ...
    Example: ``!search Date A Live`` will show all the ``Date A Live`` doujinshi.''']
        i = 0
        await bot.send_message(message.channel, 'Command number: ' + str(len(commands)))
        while i <= int(len(commands)):
            if arg == int(commands[i]):
                error = False
                await bot.send_message(message.channel, helptext[i])
                pass
            else:
                await bot.send_message(message.channel, '[DEBUG] i after: ' + str(i))
                error = True
                await bot.send_message(message.channel, '[DEBUG] error?: ' + str(error))
                i = i + 1
                await bot.send_message(message.channel, '[DEBUG] i before: ' + str(i))
        await bot.send_message(message.channel, '[DEBUG] arg: ' + arg)
        await bot.send_message(message.channel, '[DEBUG] commands: ' + str(commands).strip('[]'))
        if error is True or arg is None:
            await bot.send_message(message.channel, '''Commands aviable: ``%s``.
